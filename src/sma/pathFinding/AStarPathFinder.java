...
public class AStarPathFinder {
	 ...
	 * @param allowDiagMovement True if the search should try diaganol movement
	 */
	public AStarPathFinder(GameMap map, int maxSearchDistance, ClosestHeuristic heuristic) {
	 ...
	 */
	public Path findPath(UnitMover mover, int sx, int sy, int tx, int ty, int op) {
		// easy first check, if the destination is blocked, we can't get there
		if (map.blocked(mover, tx, ty, op)) {
			return null;
		}
		...
		while ((maxDepth < maxSearchDistance) && (open.size() != 0)) {
			...
			for (int x=-1;x<2;x++) {
				for (int y=-1;y<2;y++) {
					...
					if (isValidLocation(mover,sx,sy,xp,yp,op)) {
	 ...
	 */
	protected boolean isValidLocation(UnitMover mover, int sx, int sy, int x, int y, int option) {
		boolean invalid = (x < 0) || (y < 0) || (x >= map.getWidthInTiles()) || (y >= map.getHeightInTiles());
		
		if ((!invalid) && ((sx != x) || (sy != y))) {
			invalid = map.blocked(mover, x, y, option);
		}
